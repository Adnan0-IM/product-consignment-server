...
export class SaleService {
    static async recordSale(consigneeId, data) {
        ...
        return prisma.$transaction(async (tx) => {
            ...
            for (const item of data.items) {
                ...
                if (!consignmentItem) {
                    throw new BadRequestError(`No accepted consignment found for product ${product.name} with this consignee`);
                }
                const remainingInConsignment = consignmentItem.quantity -
                    consignmentItem.quantitySold -
                    consignmentItem.quantityReturned;
                if (remainingInConsignment < item.quantity) {
                    throw new BadRequestError(`Insufficient consigned quantity for ${product.name}. Available in consignment: ${remainingInConsignment}`);
                }
                const unitPrice = Number(product.unitPrice);
                const itemTotalPrice = unitPrice * item.quantity;
                // Calculate commission split percentages
                const consignorRate = product.consignorRate !== null
                    ? Number(product.consignorRate)
                    : env.DEFAULT_CONSIGNOR_COMMISSION;
                const consigneeRate = product.consigneeRate !== null
